...
type ToolManifest struct {
	...
	MinRhoVersion string         `json:"min_rho_version"`
}
...
// If no directories are provided, defaults to Rho user state.
...
func ParseManifest(pluginDir string) (*ToolManifest, error) {
	...
	data, err := os.ReadFile(path) // #nosec G304 -- pluginDir is a locally installed plugin directory under a Rho-managed plugins root, not raw external input
	if err != nil {
