...
// letting a hostile manifest show reviewers one value and rho another.
...
func readBoundedManifestFile(path string) ([]byte, error) {
	info, err := os.Lstat(path) // #nosec G304 -- path is pluginDir/plugin.json under a Rho-managed plugins root
	if err != nil {
