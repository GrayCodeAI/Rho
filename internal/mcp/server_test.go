...
func TestInitializeHandshake(t *testing.T) {
	server := NewMCPServer(ServerInfo{Name: "rho", Version: "1.0.0"})
	...
	if pv, typeOK := result["protocolVersion"].(string); !typeOK || pv != "2025-03-26" {
	...
	if si["name"] != "rho" {
		t.Errorf("expected server name rho, got %v", si["name"])
	}
	if si["version"] != "1.0.0" {
