...
func TestDaemon_Ready(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "engine wired without Flux probe is not ready",
			factory:    factory,
			wantStatus: http.StatusServiceUnavailable,
			wantReady:  false,
		},
...
func TestDaemon_ChatPersistsRetrievableSessionAndRequestMetadata(t *testing.T) {
	...
	if chat.SessionID == "" || resp.Header.Get("X-Rho-Session-ID") != chat.SessionID {
		t.Fatalf("session ID response/header mismatch: body=%q header=%q", chat.SessionID, resp.Header.Get("X-Rho-Session-ID"))
	}
...
func TestDaemon_ChatSSEExposesRetrievableSessionID(t *testing.T) {
	...
	id := resp.Header.Get("X-Rho-Session-ID")
