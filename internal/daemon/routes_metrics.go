...
func (s *Server) emitRuntimeMetrics(sb *strings.Builder) {
	...
	sb.WriteString("# TYPE rho_daemon_active_sessions gauge\n")
	sb.WriteString(fmt.Sprintf("rho_daemon_active_sessions %d\n", activeSessions))

	// Concurrency slots used
	sb.WriteString("# TYPE rho_daemon_chat_concurrency_used gauge\n")
	sb.WriteString(fmt.Sprintf("rho_daemon_chat_concurrency_used %d\n", len(s.concurrencySem)))

	// Uptime
	sb.WriteString("# TYPE rho_daemon_uptime_seconds gauge\n")
