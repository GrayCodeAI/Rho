...
// Provider is Rho's rho-owned view of the Flux engine: a composition of the
// role interfaces below. It wraps the concrete *fluxengine.Engine (whose fields
// are unexported and therefore not directly mockable) so Rho tests can inject a
...
type Generator interface {
	Generate(ctx context.Context, req fluxengine.GenerateRequest) (*fluxengine.GenerateResponse, error)
	Stream(ctx context.Context, req fluxengine.GenerateRequest) (fluxengine.EventStreamer, error)
}
...
type NativeCompactor interface {
	...
	CompactNative(ctx context.Context, req fluxengine.NativeCompactionRequest) (string, error)
}

// ModelCatalog is the model-discovery facet (used by routing + config).
type ModelCatalog interface {
	ListModels(ctx context.Context, providerID string, refresh bool) ([]fluxengine.Model, error)
	ListLiveModels(ctx context.Context, providerID string) ([]fluxengine.Model, error)
	ListPublicModels(ctx context.Context, providerID string) ([]fluxengine.Model, error)
	ModelInfo(ctx context.Context, modelID string) (fluxengine.Model, bool, error)
	ModelProviders(ctx context.Context) ([]string, error)
	DefaultModel(ctx context.Context, provider, fallback string) string
	PreferredModel(ctx context.Context, provider string, class fluxengine.ModelClass, fallback string) string
	PreferredModels(ctx context.Context, primaryProvider string, class fluxengine.ModelClass, limit int) []string
	ModelClassOf(ctx context.Context, modelID string) fluxengine.ModelClass
	...
	Catalog(ctx context.Context) (fluxengine.CatalogSnapshot, error)
}

// CredentialManager is the key/credential facet (config only).
type CredentialManager interface {
	SaveCredential(ctx context.Context, providerID, secret string) (fluxengine.CredentialStatus, error)
	RemoveCredential(ctx context.Context, providerID string) error
	CredentialStatus(ctx context.Context, providerID string) (fluxengine.CredentialStatus, error)
	...
	ResolveCredential(ctx context.Context, secret string) fluxengine.CredentialResolution
	CredentialProviders(context.Context) []fluxengine.CredentialProvider
	ApplyCredentials(ctx context.Context, providerID string) (fluxengine.CatalogSnapshot, error)
}

// SelectionManager is the get/set selection facet (config only).
type SelectionManager interface {
	ActiveSelection(ctx context.Context) fluxengine.Route
	EffectiveSelection(ctx context.Context, opts fluxengine.SelectionOptions) fluxengine.Selection
...
type GatewayInspector interface {
	GatewayDefinitions() []fluxengine.Gateway
	Gateways(ctx context.Context) []fluxengine.Gateway
	...
	DeploymentSummary(ctx context.Context, activeModel string) (fluxengine.DeploymentSummary, error)
...
type CatalogMaintenance interface {
	RefreshCatalog(ctx context.Context, providerID string) (fluxengine.CatalogSnapshot, error)
	CatalogHealth(ctx context.Context) fluxengine.CatalogHealth
	StatePaths() fluxengine.StatePaths
	DefaultProviderFilter(ctx context.Context) string
	PreflightWithOptions(ctx context.Context, opts fluxengine.PreflightOptions) fluxengine.PreflightReport
	ProviderStateSecurityStatus() fluxengine.ProviderStateSecurity
}

// engineProvider is the production Provider: a thin wrapper over Flux's
// concrete engine facade.
type engineProvider struct {
	eng *fluxengine.Engine
}

func newEngineProvider(eng *fluxengine.Engine) *engineProvider {
	return &engineProvider{eng: eng}
}

func (p *engineProvider) Generate(ctx context.Context, req fluxengine.GenerateRequest) (*fluxengine.GenerateResponse, error) {
	return p.eng.Generate(ctx, req)
}

func (p *engineProvider) Stream(ctx context.Context, req fluxengine.GenerateRequest) (fluxengine.EventStreamer, error) {
	return p.eng.Stream(ctx, req)
}

func (p *engineProvider) ListModels(ctx context.Context, providerID string, refresh bool) ([]fluxengine.Model, error) {
	return p.eng.ListModels(ctx, providerID, refresh)
}

func (p *engineProvider) ListLiveModels(ctx context.Context, providerID string) ([]fluxengine.Model, error) {
	return p.eng.ListLiveModels(ctx, providerID)
}

func (p *engineProvider) ListPublicModels(ctx context.Context, providerID string) ([]fluxengine.Model, error) {
	return p.eng.ListPublicModels(ctx, providerID)
}

func (p *engineProvider) ModelInfo(ctx context.Context, modelID string) (fluxengine.Model, bool, error) {
...
func (p *engineProvider) PreferredModel(ctx context.Context, provider string, class fluxengine.ModelClass, fallback string) string {
	return p.eng.PreferredModel(ctx, provider, class, fallback)
}

func (p *engineProvider) PreferredModels(ctx context.Context, primaryProvider string, class fluxengine.ModelClass, limit int) []string {
	return p.eng.PreferredModels(ctx, primaryProvider, class, limit)
}

func (p *engineProvider) ModelClassOf(ctx context.Context, modelID string) fluxengine.ModelClass {
...
func (p *engineProvider) StatePaths() fluxengine.StatePaths {
...
func (p *engineProvider) Catalog(ctx context.Context) (fluxengine.CatalogSnapshot, error) {
	return p.eng.Catalog(ctx)
}

func (p *engineProvider) RefreshCatalog(ctx context.Context, providerID string) (fluxengine.CatalogSnapshot, error) {
	return p.eng.RefreshCatalog(ctx, providerID)
}

func (p *engineProvider) ApplyCredentials(ctx context.Context, providerID string) (fluxengine.CatalogSnapshot, error) {
	return p.eng.ApplyCredentials(ctx, providerID)
}

func (p *engineProvider) SaveCredential(ctx context.Context, providerID, secret string) (fluxengine.CredentialStatus, error) {
...
func (p *engineProvider) CredentialStatus(ctx context.Context, providerID string) (fluxengine.CredentialStatus, error) {
...
func (p *engineProvider) ResolveCredential(ctx context.Context, secret string) fluxengine.CredentialResolution {
	return p.eng.ResolveCredential(ctx, secret)
}

func (p *engineProvider) CredentialProviders(ctx context.Context) []fluxengine.CredentialProvider {
	return p.eng.CredentialProviders(ctx)
}

func (p *engineProvider) GatewayDefinitions() []fluxengine.Gateway {
	return p.eng.GatewayDefinitions()
}

func (p *engineProvider) Gateways(ctx context.Context) []fluxengine.Gateway {
...
func (p *engineProvider) DeploymentSummary(ctx context.Context, activeModel string) (fluxengine.DeploymentSummary, error) {
...
func (p *engineProvider) CatalogHealth(ctx context.Context) fluxengine.CatalogHealth {
	return p.eng.CatalogHealth(ctx)
}

func (p *engineProvider) PreflightWithOptions(ctx context.Context, opts fluxengine.PreflightOptions) fluxengine.PreflightReport {
	return p.eng.PreflightWithOptions(ctx, opts)
}

func (p *engineProvider) ActiveSelection(ctx context.Context) fluxengine.Route {
	return p.eng.ActiveSelection(ctx)
}

func (p *engineProvider) EffectiveSelection(ctx context.Context, opts fluxengine.SelectionOptions) fluxengine.Selection {
...
func (p *engineProvider) ProviderStateSecurityStatus() fluxengine.ProviderStateSecurity {
...
func (p *engineProvider) CompactNative(ctx context.Context, req fluxengine.NativeCompactionRequest) (string, error) {
	return p.eng.CompactNative(ctx, req)
}

// translateProvider bridges the rho-owned ChatClient port to the Generator and
// NativeCompactor roles. It needs no other Provider facet. The type conversions
// here (internal/types <-> fluxengine.*) are the single, centralized
// translation point — Rho's conversation DTOs never leak past it.
...
func (c *translateProvider) Chat(ctx context.Context, messages []types.FluxMessage, opts types.ChatOptions) (*types.FluxResponse, error) {
...
func (c *translateProvider) StreamChatContinue(ctx context.Context, messages []types.FluxMessage, opts types.ChatOptions, continuation types.ContinuationConfig) (*types.StreamResult, error) {
	...
	events := make(chan types.FluxStreamEvent, 64)
	...
	go func() {
		...
		if err := stream.Err(); err != nil {
			select {
			case events <- types.FluxStreamEvent{Type: "error", Error: err.Error()}:
			case <-streamCtx.Done():
			}
		}
	}()
...
// ManagesResilience tells Rho not to add provider retry, continuation, or
// protocol-recovery layers around Flux's routed transport.
...
func (c *translateProvider) CompactNative(ctx context.Context, req fluxengine.NativeCompactionRequest) (string, error) {
	if c.compactor == nil {
...
func toEngineRequest(messages []types.FluxMessage, opts types.ChatOptions, continuation types.ContinuationConfig) fluxengine.GenerateRequest {
	thinkingEnabled := opts.ThinkingEnabled
	if thinkingEnabled == nil {
		thinkingEnabled = opts.GLMThinkingEnabled
	}
	reasoningEnabled := thinkingEnabled != nil && *thinkingEnabled
	request := fluxengine.GenerateRequest{
		Messages:     toEngineMessages(messages),
		SystemPrompt: opts.System,
		Tools:        toEngineTools(opts.Tools),
		Requirements: fluxengine.Requirements{
...
// ToEngineMessages returns the messages unchanged: rho, the engine, and the
...
func ToEngineMessages(messages []types.FluxMessage) []fluxengine.Message {
	return messages
}
...
func toEngineTools(tools []types.FluxTool) []fluxengine.Tool {
	return tools
}

func toEngineToolChoice(choice *types.ToolChoiceOption) *fluxengine.ToolChoice {
	return choice
}

func fromEngineResponse(response *fluxengine.GenerateResponse) *types.FluxResponse {
	return response
}

func fromEngineEvent(event fluxengine.Event) (types.FluxStreamEvent, bool) {
	out := types.FluxStreamEvent{
	...
	switch event.Type {
	case fluxengine.EventRouteSelected:
		out.Type = "route_selected"
	case fluxengine.EventRouteChanged:
		out.Type = "route_changed"
	case fluxengine.EventContentDelta:
		out.Type = "content"
	case fluxengine.EventThinkingDelta:
		out.Type = "thinking"
	case fluxengine.EventToolCallStart, fluxengine.EventToolCallDone:
		out.Type = "tool_call"
	case fluxengine.EventToolCallDelta:
		out.Type = "tool_input_delta"
	case fluxengine.EventUsage:
		out.Type = "usage"
	case fluxengine.EventTTFT:
		out.Type = "ttft"
		out.TTFT = event.TTFTms
	case fluxengine.EventDone:
		out.Type = "done"
	case fluxengine.EventContinuation:
		out.Type = "continuation"
	case fluxengine.EventWarning:
		out.Type, out.Content = "warning", event.Warning
	default:
		// An unrecognized engine event type means flux emits something this
...
func messagesContainVision(messages []types.FluxMessage) bool {
