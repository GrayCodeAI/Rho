...
func TestConfiguredNone(t *testing.T) {
	withEnv(t, map[string]string{"RHO_NOTIFY_WEBHOOK_URL": "", "RHO_NOTIFY_TELEGRAM_TOKEN": ""})
	if Configured() {
...
func TestSendWebhookPayload(t *testing.T) {
	...
	defer srv.Close()
	t.Setenv("RHO_NOTIFY_WEBHOOK_URL", srv.URL)

	if err := SendCompletion(Completion{Title: "done", Body: "built it", OK: true, Source: "rho exec", Branch: "b1"}); err != nil {
