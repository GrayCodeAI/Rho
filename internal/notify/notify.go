...
type Completion struct {
	...
	OK         bool   `json:"ok"`
	Source     string `json:"source,omitempty"` // e.g. "rho exec"
...
func Configured() bool {
	return envReader("RHO_NOTIFY_WEBHOOK_URL") != "" ||
		(envReader("RHO_NOTIFY_TELEGRAM_TOKEN") != "" && envReader("RHO_NOTIFY_TELEGRAM_CHAT_ID") != "")
}
...
func SendCompletion(c Completion) error {
	...
	var errs []string
	if url := envReader("RHO_NOTIFY_WEBHOOK_URL"); url != "" {
		if err := sendWebhook(url, c); err != nil {
			errs = append(errs, "webhook: "+err.Error())
		}
	}
	tok, chat := envReader("RHO_NOTIFY_TELEGRAM_TOKEN"), envReader("RHO_NOTIFY_TELEGRAM_CHAT_ID")
	if tok != "" && chat != "" {
