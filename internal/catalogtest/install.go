...
// InstallGlobal writes the test catalog to a temp file and sets FLUX_MODEL_CATALOG_PATH.
// Call from TestMain; returns cleanup to unset env.
func InstallGlobal() (cleanup func()) {
	globalOnce.Do(func() {
		dir, err := os.MkdirTemp("", "rho-catalog-*")
		if err != nil {
		...
		_ = os.Setenv("FLUX_MODEL_CATALOG_PATH", globalPath)
	})
	return func() {
		_ = os.Unsetenv("FLUX_MODEL_CATALOG_PATH")
	}
}

// Install sets FLUX_MODEL_CATALOG_PATH for a single test (per-test temp file).
func Install(t testing.TB) {
	...
	t.Setenv("FLUX_MODEL_CATALOG_PATH", path)
}
