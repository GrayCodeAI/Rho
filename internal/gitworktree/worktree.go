...
// Create makes a new git worktree under the repo's .rho/worktrees directory.
...
func Create(ctx context.Context, repoDir, branch string) (path string, cleanup func(), err error) {
	...
	base := filepath.Join(repoDir, ".rho", "worktrees")
	if err := os.MkdirAll(base, 0o700); err != nil {
		return "", nil, err
	}
	if branch == "" {
		branch = fmt.Sprintf("rho-subagent-%d", time.Now().UnixNano())
	}
