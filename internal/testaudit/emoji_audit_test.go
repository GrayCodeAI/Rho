...
//   - welcome_banner.go — RHO block-letter logo, U+2588 block elements.
...
var emojiAuditPathExempt = []string{
	...
	// parser vocabulary. The literals are not rho's own output; they
	...
	// captured external tool output, not rho's own rendering.
	...
	// rho's rendering, so it is recognised rather than produced.
...
// TestNoEmojiInCmd enforces rho's "no emoji in CLI" rule. Every glyph
// that rho renders must come from internal/ui/icons. This test scans
