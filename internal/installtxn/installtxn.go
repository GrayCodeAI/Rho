...
//
// Adopted from Zero (internal/installtxn) — atomic install with rollback.
package installtxn
...
const lockFileName = ".rho-install.lock"
...
func StageDir(dir string) (stage string, cleanup func(), err error) {
	...
	workspace, err := os.MkdirTemp(dir, ".rho-install-txn-")
	if err != nil {
...
func RemoveDir(target string, publish func() error) error {
	workspace, err := os.MkdirTemp(filepath.Dir(target), ".rho-install-txn-")
	if err != nil {
...
func WriteFileAtomically(path string, data []byte, perm os.FileMode) error {
	temp, err := os.CreateTemp(filepath.Dir(path), ".rho-lockfile-")
	if err != nil {
		return err
	}
	tempPath := temp.Name()
