...
// ConversationNode is one message in Rho's product-owned conversation graph.
...
func OpenConversationGraph(path, sessionID string) (*ConversationGraph, error) {
	...
	data, err := os.ReadFile(path) // #nosec G304 -- path is supplied by Rho's session storage composition root
	if err == nil {
