...
func TestSpecifyWritesSpecFile(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("expected .rho/specs directory to exist: %v", err)
	}
	if len(entries) != 1 {
	...
	content, err := os.ReadFile(filepath.Join(dir, ".rho", "specs", entries[0].Name(), "spec.md"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestSpecifyPlanTasksSequence(t *testing.T) {
	...
	specPath := filepath.Join(dir, ".rho", "specs", entries[0].Name())
