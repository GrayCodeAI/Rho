...
import (
	...
	homepkg "github.com/GrayCodeAI/rho/internal/home"
)
...
var (
	...
	rhoEnvReadRe            = regexp.MustCompile(`(?i)\b(cat|type|head|less|more|dd)\b[^\n;|]*\.rho/(env|\.env)\b`)
...
func (BashTool) Execute(ctx context.Context, input json.RawMessage) (string, error) {
	...
	// Shared safety stack: destructive-command, AST, and obfuscation checks.
	if err := validateShellCommand(ctx, p.Command); err != nil {
		return "", err
	}

	// Explore/plan hard gate: background bash can still mutate via
	// long-running processes; deny. The read-only allowlist itself is
	// enforced inside validateShellCommand.
	if tc := GetToolContext(ctx); tc != nil && tc.ReadOnlyBash && p.RunInBackground {
