...
// should be blocked for security. The policy lives in internal/pathsafe so
// low-level packages can consult it without importing tool.
func IsSensitivePath(path string) string {
	return pathsafe.IsSensitivePath(path)
}
...
func CommandReferencesSensitivePath(command string) string {
	...
	for _, tok := range strings.FieldsFunc(command, commandPathSeparators) {
		...
		for _, suffix := range pathsafe.BlockedPathSuffixes {
		...
		if strings.HasSuffix(tok, ".rho/provider.json") {
		...
		for _, b := range pathsafe.BlockedBasenames {
			if base == b {
...
func ResolvePath(path string) (string, error) {
	return pathsafe.ResolvePath(path)
}
