...
func TestExtractTicketRef_BranchName(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			branch:   "feature/RHO-123-add-auth",
			prDesc:   "",
			expected: []string{"RHO-123"},
		},
		{
...
func TestExtractTicketRef_PRDescription(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			prDesc:   "Resolves RHO-99",
			contains: []string{"RHO-99"},
		},
		{
			name:     "multiple references",
			prDesc:   "Fixes #42 and also resolves RHO-99\nRelated to PROJ-200",
			contains: []string{"#42", "RHO-99", "PROJ-200"},
		},
		{
			name:     "no references",
...
func TestExtractTicketRef_Combined(t *testing.T) {
	...
	refs := tc.ExtractTicketRef("feature/RHO-123-jwt", "Fixes #42\nResolves RHO-123")
	// RHO-123 should only appear once.
	count := 0
	for _, r := range refs {
		if r == "RHO-123" {
			count++
	...
	if count != 1 {
		t.Errorf("expected RHO-123 to appear once, appeared %d times in %v", count, refs)
	}
...
func TestFormatComplianceResult(t *testing.T) {
	...
	if !strings.Contains(output, "RHO-123") {
...
func TestTicketComplianceTool_Execute(t *testing.T) {
	...
	input := map[string]string{
		"branch_name":    "feature/RHO-123-jwt",
		"pr_description": "Fixes RHO-123\n\nAdds JWT authentication",
...
`,
	...
	if !strings.Contains(result, "RHO-123") {
