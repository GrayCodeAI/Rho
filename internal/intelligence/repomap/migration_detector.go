...
// builtinRules returns the static migration-rule catalog. A fresh slice is
// returned so callers can filter or annotate it without mutating the catalog.
func builtinRules() []MigrationRule {
	rules := make([]MigrationRule, len(builtinMigrationRules))
	copy(rules, builtinMigrationRules)
	return rules
}

// builtinMigrationRules is the static migration-rule catalog.
