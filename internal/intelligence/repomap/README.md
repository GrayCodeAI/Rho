...
> Deep code-analysis engine for rho: language-aware symbol extraction,
...
/fsnotify` for file watching; rho's `internal/scoring` and
...
- **Persistent incremental cache** (`incremental_map.go`): JSON file at
  `<cacheDir>/repomap-cache.json` (typically `.rho/repomap-cache.json`)
...
that rho's context layer calls when it needs a budgeted overview for the
