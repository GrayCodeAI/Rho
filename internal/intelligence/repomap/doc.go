// Package repomap is the deep code-analysis engine that powers rho's
...
// /fsnotify for file watching, plus rho's own internal/scoring and
...
//
// rho ships a second package, internal/context/repomap, that exposes a much
// narrower surface - essentially just RepoMap(root, budget) (string, error).
// That package is the prompt-injection shim: it is what rho's context layer
...
// .rho/repomap-cache.json keyed by SHA-256. Callers that need to operate on
