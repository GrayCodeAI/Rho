...
func (s *Session) smartCompactBody(ctx context.Context) {
	...
	if sessionID := s.executionGraphSessionID(); sessionID != "" && len(compactedMsgs) > 0 {
		detail, _ := compact.ParseCompactionDetail(os.Getenv("RHO_COMPACTION_SEGMENT_DETAIL"))
	...
	keep := make([]types.FluxMessage, 0, len(tail)+2)
...
func (s *Session) generateSummary(ctx context.Context, raw []types.FluxMessage) string {
	...
	// Try token-engine compression first as a fast, zero-cost alternative
...
func extractSummaryFromCompressed(compressed string) string {
	...
	// Token compression preserves semantic meaning; extract an actionable summary
...
// Uses the embedded token engine for fast, zero-cost compression. Returns the original if already short enough.
...
// Queries flux's catalog at runtime — no hardcoded model names.
