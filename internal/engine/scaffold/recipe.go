...
func (r *RecipeRegistry) Encode(recipe *Recipe) string {
	...
	return "rho://recipe/" + encoded
}

// Decode parses a rho:// deeplink URL, base64url-decodes the payload,
// and deserializes it into a Recipe.
func (r *RecipeRegistry) Decode(deeplink string) (*Recipe, error) {
	const prefix = "rho://recipe/"
	if !strings.HasPrefix(deeplink, prefix) {
		return nil, errors.New("invalid deeplink: must start with rho://recipe/")
	}
...
// ImportFromURL decodes a rho:// deeplink URL and imports the recipe into
// the registry.
