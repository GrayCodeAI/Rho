...
func ExtractPriorSummary(msgs []types.FluxMessage) string {
...
func AdjustIndexToPreserveAPIInvariants(msgs []types.FluxMessage, startIdx int) int {
	return compact.AdjustIndexToPreserveAPIInvariants(msgs, startIdx)
}
func HasTextContent(m types.FluxMessage) bool { return compact.HasTextContent(m) }
