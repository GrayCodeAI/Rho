...
func TestNewCostOptimizer(t *testing.T) {
	...
	if len(co.ModelPricing) != 8 {
		t.Errorf("expected 8 model pricings, got %d", len(co.ModelPricing))
	}
...
func TestWhatIf(t *testing.T) {
	...
	if haikuCost > sonnetCost {
		t.Errorf("WhatIf haiku (%.4f) should not be more expensive than sonnet (%.4f)", haikuCost, sonnetCost)
	}
}
