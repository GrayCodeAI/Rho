...
// Flux client, provider/model identity, and compatibility-only rate/retry
// controls. Production facade clients own resilience inside Flux. The service
...
type ChatService struct {
	mu sync.RWMutex
	// client is the flux transport. Always non-nil after construction.
	client ChatClient
	// provider / model are the active LLM identity.
	provider string
	model    string
	// deploymentRouting is true when the client is catalog-backed
	// (e.g. DeploymentRouter from flux/runtime.ChatProvider).
...
// Client returns the underlying flux client. Exposed for callers (e.g.
...
func (c *ChatService) BuildOptions(systemPrompt, activeModel string, maxTokens int, tools []types.FluxTool) types.ChatOptions {
	c.mu.RLock()
	provider := c.provider
	...
	// Opt-in tool-catalog compression (RHO_TOOL_SHRINK=1): fail-open, so the
	// returned tools equal the input whenever anything is off or drifts.
	opts.Tools = shrinkFluxTools(opts.Tools)
	return opts
}
...
// emits FluxStreamEvent values; the caller must Close() the result
// when done.
//
...
// check ctx.Err()).
//
// Flux facade clients advertise that they manage provider resilience. For
// those clients this service records the product metric and delegates exactly
// once; injected legacy clients retain Rho's compatibility retry/rate layer.
func (c *ChatService) Stream(ctx context.Context, messages []types.FluxMessage, opts types.ChatOptions) (*types.StreamResult, error) {
	c.mu.RLock()
	client := c.client
...
func emergencyCompact(messages []types.FluxMessage) []types.FluxMessage {
	if len(messages) <= emergencyCompactMin {
		return messages
	}
	out := make([]types.FluxMessage, 0, emergencyCompactWindow+1)
...
// incremental events.
func (c *ChatService) Chat(ctx context.Context, messages []types.FluxMessage, opts types.ChatOptions) (*types.FluxResponse, error) {
	client := c.Client()
	if client == nil {
		return nil, errors.New("chat service: no client configured")
	}
