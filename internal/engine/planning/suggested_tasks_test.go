...
func TestFormatTasks(t *testing.T) {
	tasks := []*SuggestedTask{
		{
			...
			Priority: 1,
			Category: "fix",
			Command:  `rho exec "resolve the merge conflict"`,
		},
		{
			ID:       "t2",
			...
			Priority: 2,
			Category: "test",
			Command:  `rho exec "fix the failing test"`,
		},
		{
			ID:       "t3",
			...
			Priority: 4,
			Category: "docs",
			Command:  `rho exec "add godoc to ParseConfig"`,
		},
	}
	...
	if !strings.Contains(result, "Run: rho exec") {
