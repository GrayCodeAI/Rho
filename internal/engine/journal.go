...
func (s *PersistenceService) AppendUserJournaled(msg types.FluxMessage) {
	if s == nil {
		return
	}
...
func (s *PersistenceService) AppendAssistantJournaled(msg types.FluxMessage) {
	if s == nil {
		return
	}
	if j := s.Journal(); j != nil {
...
func (s *PersistenceService) JournalProjection() []types.FluxMessage {
	if s == nil {
		return nil
	}
	j := s.Journal()
...
func toJournalMessage(m types.FluxMessage) eventlog.Message {
	jm := eventlog.Message{
		Role:     m.Role,
		Content:  m.Content,
		Thinking: m.Thinking,
...
func fromJournalMessage(m eventlog.Message) types.FluxMessage {
	out := types.FluxMessage{
		Role:     m.Role,
		Content:  m.Content,
		Thinking: m.Thinking,
		Images:   append([]string(nil), m.Images...),
...
func cloneSingleMessage(m types.FluxMessage) types.FluxMessage {
	clones := cloneMessages([]types.FluxMessage{m})
	return clones[0]
}
