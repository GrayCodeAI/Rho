...
func CollapseRepeatedMessages(msgs []types.FluxMessage) []types.FluxMessage {
	if len(msgs) < 3 {
		return msgs
	}

	result := make([]types.FluxMessage, 0, len(msgs))
	...
	for i < len(msgs) {
		...
		if isErrorMessage(msgs[i]) {
			...
			if runLen >= 2 {
				...
				result = append(result, types.FluxMessage{
					Role:        msgs[i].Role,
...
func isSimilarToolResult(a, b types.FluxMessage) bool {
...
func toolResultSource(msg types.FluxMessage) string {
...
func isErrorMessage(msg types.FluxMessage) bool {
...
func extractErrorText(msg types.FluxMessage) string {
	if len(msg.ToolResults) > 0 && msg.ToolResults[0].IsError {
