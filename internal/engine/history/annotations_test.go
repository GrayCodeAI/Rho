...
func TestAnnotationManager_InjectAnnotationsPython(t *testing.T) {
	...
	if !strings.Contains(result, "# [rho:todo] Add type hints") {
...
func TestAnnotationManager_InjectAnnotationsJS(t *testing.T) {
	...
	if !strings.Contains(result, "// [rho:warning] Potential XSS") {
...
func TestAnnotationManager_InjectAnnotationsResolved(t *testing.T) {
	...
	if strings.Contains(result, "[rho:") {
...
func TestAnnotationManager_InjectAnnotationsMultiple(t *testing.T) {
	...
	if lines[0] != "// [rho:note] First note" {
	...
	if lines[3] != "// [rho:todo] Second note" {
...
// [rho:note] This is a note
func main() {
	// [rho:warning] Watch out
	fmt.Println("hello")
}
`
	...
	if strings.Contains(result, "[rho:") {
		t.Fatalf("expected all rho annotations stripped, got:\n%s", result)
	}
...
func TestAnnotation_StripAnnotationsPython(t *testing.T) {
	content := `# [rho:todo] Add type hints
def hello():
    pass
`
	result := StripAnnotations(content)
	...
	if strings.Contains(result, "[rho:") {
		t.Fatalf("expected rho annotation stripped, got:\n%s", result)
	}
...
// [rho:note] Claims struct could be simplified
func main() {
	// [rho:warning] No error check after ParseToken()
	token := ParseToken()
	// [rho:todo] Add rate limiting here
	handle(token)
}
`
...
func TestAnnotation_DetectAnnotationsPython(t *testing.T) {
	content := "# [rho:question] Why is this needed?\ndef func():\n    pass\n"
	anns := DetectAnnotations(content)
	if len(anns) != 1 {
		t.Fatalf("expected 1 annotation, got %d", len(anns))
	}
...
func TestAnnotation_StripAndInjectRoundTrip(t *testing.T) {
	...
	if !strings.Contains(injected, "[rho:note]") {
		t.Fatal("expected annotations in injected content")
	}
	if !strings.Contains(injected, "[rho:todo]") {
		t.Fatal("expected annotations in injected content")
	}
