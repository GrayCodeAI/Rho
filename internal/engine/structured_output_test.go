...
func TestChatStructured_NoSchema(t *testing.T) {
	...
	resp, err := s.ChatStructured(context.Background(),
		[]types.FluxMessage{{Role: "user", Content: "hi"}}, types.ChatOptions{}, "")
	if err != nil {
...
func TestChatStructured_ValidFirstTry(t *testing.T) {
	...
	_, err := s.ChatStructured(context.Background(),
		[]types.FluxMessage{{Role: "user", Content: "describe ada"}}, types.ChatOptions{}, personSchema)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if mc.callCount() != 1 {
...
func TestChatStructured_RetryOnceThenReturnsError(t *testing.T) {
	...
	resp, err := s.ChatStructured(context.Background(),
		[]types.FluxMessage{{Role: "user", Content: "describe"}}, types.ChatOptions{}, personSchema)
	if err == nil {
