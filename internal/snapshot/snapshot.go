...
// The shadow git repository lives under Rho's user state snapshots directory
// rather than under projectDir, so that running rho from inside a Go project
// root no longer creates a nested <cwd>/cmd/.rho/ tree at runtime.
...
func (t *Tracker) Init() error {
	...
	if err := t.gitWork("config", "user.email", "rho@snapshot"); err != nil {
		return fmt.Errorf("config email: %w", err)
	}
	if err := t.gitWork("config", "user.name", "rho-snapshot"); err != nil {
