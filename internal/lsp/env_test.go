...
func TestScrubEnvironment_NilAmbient(t *testing.T) {
	os.Setenv("RHO_TEST_SAFE_VAR", "safe_value")
	os.Setenv("RHO_TEST_SECRET_KEY", "hidden_secret")
	defer os.Unsetenv("RHO_TEST_SAFE_VAR")
	defer os.Unsetenv("RHO_TEST_SECRET_KEY")
	...
	for _, entry := range scrubbed {
		if strings.HasPrefix(entry, "RHO_TEST_SAFE_VAR=") {
			foundSafe = true
		}
		if strings.HasPrefix(entry, "RHO_TEST_SECRET_KEY=") {
			foundSecret = true
		}
	...
	if !foundSafe {
		t.Error("expected RHO_TEST_SAFE_VAR to be present")
	}
	if foundSecret {
		t.Error("expected RHO_TEST_SECRET_KEY to be scrubbed")
	}
	if !foundInjected {
