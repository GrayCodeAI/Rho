// Package env provides environment helpers for rho's process management.
package env
...
// inherit. It mirrors the provider key names in Flux's provider profiles
// (flux/config/profiles.go). Keeping the list explicit here avoids
// importing flux internals and makes the boundary auditable.
var ScrubSet = []string{
	"ANTHROPIC_API_KEY",
	"AWS_ACCESS_KEY_ID",
