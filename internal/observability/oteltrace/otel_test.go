...
func TestDefaultTelemetryConfig(t *testing.T) {
	os.Setenv("RHO_ENABLE_TELEMETRY", "1")
	defer os.Unsetenv("RHO_ENABLE_TELEMETRY")

	cfg := DefaultTelemetryConfig()
	if !cfg.Enabled {
		t.Error("expected telemetry enabled when env var set")
	}
	if cfg.ServiceName != "rho" {
		t.Errorf("expected service name rho, got %s", cfg.ServiceName)
	}
...
func TestDefaultTelemetryConfig_Disabled(t *testing.T) {
	os.Unsetenv("RHO_ENABLE_TELEMETRY")
...
// implicitly enable rho telemetry (Phase 3 hardening).
