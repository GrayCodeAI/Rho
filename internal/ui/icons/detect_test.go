...
func TestMode_RHOIconsNerdOverrides(t *testing.T) {
	t.Setenv("RHO_ICONS", "nerd")
	t.Setenv("TERM", "dumb")
	t.Setenv("NO_COLOR", "1")
	// Even with NO_COLOR and a dumb TERM, RHO_ICONS=nerd wins.
	withInjectedTTY(t, true)
	SetMode(ModeAuto)
	if Mode() != ModeNerd {
		t.Errorf("RHO_ICONS=nerd should win, got %s", Mode())
	}
}

func TestMode_RHOIconsAsciiOverrides(t *testing.T) {
	t.Setenv("RHO_ICONS", "ascii")
	t.Setenv("TERM", "xterm-256color")
	withInjectedTTY(t, true)
	SetMode(ModeAuto)
	if Mode() != ModeASCII {
		t.Errorf("RHO_ICONS=ascii should win, got %s", Mode())
	}
}
