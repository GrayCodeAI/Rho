...
func (m *Mission) buildPortableGraph(generatedAt time.Time) (executiongraph.Export, error) {
	...
	missionProvenance := portableProvenance(m.ID, "rho://mission/"+m.ID)
	...
	if err := portableAddEvent(&export, graphcontracts.Event{
		ID:             "rho/event/mission/" + m.ID + "/created",
		Type:           graphcontracts.EventCreated,
		Subject:        missionRef,
		Scope:          scope,
		OccurredAt:     missionCreated,
		CorrelationID:  m.ID,
		IdempotencyKey: "rho/mission/" + m.ID + "/created",
		Provenance:     missionProvenance,
	}); err != nil {
		return executiongraph.Export{}, err
	}
	if !m.CompletedAt.IsZero() {
		if err := portableAddEvent(&export, graphcontracts.Event{
			ID:             "rho/event/mission/" + m.ID + "/transitioned",
			Type:           graphcontracts.EventTransitioned,
			Subject:        missionRef,
			Scope:          scope,
			OccurredAt:     m.CompletedAt.UTC(),
			CorrelationID:  m.ID,
			CausationID:    "rho/event/mission/" + m.ID + "/created",
			IdempotencyKey: "rho/mission/" + m.ID + "/transitioned/" + string(m.Status),
			Provenance:     missionProvenance,
		}); err != nil {
			return executiongraph.Export{}, err
		}
	...
	for _, feature := range m.Features {
		...
		provenance := portableProvenance(featureID, "rho://mission/"+m.ID+"/feature/"+featureID)
		...
		if err := portableAddEdge(&export, graphcontracts.Edge{
			ID:         "rho/edge/mission/" + m.ID + "/contains/" + featureID,
		...
		if err := portableAddEvent(&export, graphcontracts.Event{
			ID:             "rho/event/mission-feature/" + m.ID + "/" + featureID + "/observed",
			Type:           graphcontracts.EventObserved,
			Subject:        ref,
			Scope:          scope,
			OccurredAt:     portableGraphTime(feature.CompletedAt, createdAt),
			CorrelationID:  m.ID,
			IdempotencyKey: "rho/mission-feature/" + m.ID + "/" + featureID + "/observed/" + string(feature.Status),
			Provenance:     provenance,
		}); err != nil {
			return executiongraph.Export{}, err
		}
	...
	for _, join := range m.WaveJoins {
		...
		provenance := portableProvenance(waveID, "rho://mission/"+m.ID+"/wave/"+waveID)
		node := graphcontracts.Node{
			ID:          ref.ID,
			Kind:        ref.Kind,
			Scope:       scope,
		...
		if err := portableAddEdge(&export, graphcontracts.Edge{
			ID:         "rho/edge/mission/" + m.ID + "/produced-wave/" + waveID,
		...
		for _, featureID := range join.FeatureIDs {
			...
			if err := portableAddEdge(&export, graphcontracts.Edge{
				ID:         "rho/edge/mission-wave/" + m.ID + "/" + waveID + "/contains/" + featureID,
				Kind:       graphcontracts.EdgeContains,
				From:       ref,
				To:         target,
				Scope:      scope,
		...
		if err := portableAddEvent(&export, graphcontracts.Event{
			ID:             "rho/event/mission-wave/" + m.ID + "/" + waveID + "/observed",
			Type:           graphcontracts.EventObserved,
			Subject:        ref,
			Scope:          scope,
			OccurredAt:     completedAt,
			CorrelationID:  m.ID,
			IdempotencyKey: "rho/mission-wave/" + m.ID + "/" + waveID + "/observed",
			Provenance:     provenance,
		}); err != nil {
			return executiongraph.Export{}, err
		}
...
func portableMissionNodeID(missionID string) string {
	return "rho/mission/" + strings.TrimSpace(missionID)
}

func portableFeatureNodeID(missionID, featureID string) string {
	return "rho/mission-feature/" + strings.TrimSpace(missionID) + "/" + strings.TrimSpace(featureID)
}

func portableWaveNodeID(missionID string, wave int) string {
	return "rho/mission-wave/" + strings.TrimSpace(missionID) + "/" + strconv.Itoa(wave)
}
