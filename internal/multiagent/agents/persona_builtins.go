...
func BuiltinPersonas() []*Persona {
	now := time.Now()
	personas := make([]*Persona, 0, len(builtinPersonaSpecs))
	for i := range builtinPersonaSpecs {
		p := builtinPersonaSpecs[i]
		p.CreatedAt = now
		// Clone slice fields so callers cannot mutate the shared catalog.
		p.Tools = append([]string(nil), p.Tools...)
		p.ExcludedTools = append([]string(nil), p.ExcludedTools...)
		p.Expertise = append([]string(nil), p.Expertise...)
		p.Rules = append([]string(nil), p.Rules...)
		p.Examples = append([]PersonaExample(nil), p.Examples...)
		personas = append(personas, &p)
	}
	return personas
}

// builtinPersonaSpecs is the static built-in persona catalog. Timestamps are
// stamped per call by BuiltinPersonas.
var builtinPersonaSpecs = []Persona{
	{
		Name:               "default",
	...
	// --- Cavecrew personas (built into GrayCode Rho) ---
...
// (investigator, builder, reviewer) built into GrayCode Rho.
