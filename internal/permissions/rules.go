...
// LoadFromFile parses a .rho/rules file and populates the RuleSet.
func (rs *RuleSet) LoadFromFile(path string) error {
	f, err := os.Open(path) // #nosec G304 -- path is the caller-supplied .rho/rules config path, not external input
	if err != nil {
...
// SaveToFile writes the rules to a file in the .rho/rules format.
func (rs *RuleSet) SaveToFile(path string) error {
	...
	f, err := os.Create(path) // #nosec G304 -- path is caller-supplied .rho/rules file location, not external untrusted input
	if err != nil {
