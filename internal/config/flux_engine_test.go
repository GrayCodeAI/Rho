...
func TestNewFluxEngineForSettingsIsInvocationScoped(t *testing.T) {
	...
	if _, collisionErr := NewFluxEngineForSettings(Settings{CustomProviders: []CustomProviderConfig{{
	...
	other, err := NewFluxEngineForSettings(Settings{})
	if err != nil {
		t.Fatal(err)
	}
