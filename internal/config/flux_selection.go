...
// ActiveModel returns the selected model from flux provider.json (not rho settings).
...
// ActiveProvider returns the selected provider from flux provider.json.
...
// ActiveProviderID canonicalizes a host-facing provider/gateway id through Flux runtime.
func ActiveProviderID(provider string) string {
	return gateway.NormalizeProviderID(provider)
}

// SetActiveModel persists model selection to flux provider.json.
...
// SetActiveProvider persists provider selection to flux provider.json.
