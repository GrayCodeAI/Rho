...
func TestKeyDeterministic(t *testing.T) {
	msgs := []types.FluxMessage{{Role: "user", Content: "hello"}}
...
func TestPutGetResponseRoundTrip(t *testing.T) {
	c := New(t.TempDir())
	key := Key("fp", "p", "m", []types.FluxMessage{{Role: "user", Content: "hi"}}, 10)
	want := &types.FluxResponse{Content: "cached answer", FinishReason: "end_turn"}
...
func TestStreamRoundTrip(t *testing.T) {
	c := New(t.TempDir())
	key := Key("fp", "p", "m", []types.FluxMessage{{Role: "user", Content: "s"}}, 10)
	events := []types.FluxStreamEvent{
...
func TestFilesAreNotWorldReadable(t *testing.T) {
	...
	if err := c.Put(key, &types.FluxResponse{Content: "x"}); err != nil {
		t.Fatal(err)
	}
...
func TestEntriesCountsBothKinds(t *testing.T) {
	c := New(t.TempDir())
	k := Key("fp", "p", "m", []types.FluxMessage{{Role: "user", Content: "e"}}, 5)
	if err := c.Put(k, &types.FluxResponse{Content: "r"}); err != nil {
		t.Fatal(err)
	}
	if err := c.PutStream(k, []types.FluxStreamEvent{{Type: "done"}}); err != nil {
		t.Fatal(err)
	}
