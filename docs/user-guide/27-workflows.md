...
- `/command` invokes a Rho slash command.
...
```bash
rho review run HEAD --output-format json --max-turns 8
```
...
2. Merlin configured MCP servers with `rho mcp`.
...
from interruptions. Rho trims incomplete tool turns before a cancelled session
