...
Rho draws all TUI colors from a central theme. You can switch themes while running, follow your operating system's light or dark appearance, and adjust scroll speed and compact mode through slash commands or settings.

---

## Available Themes

Rho includes 17 built-in themes, plus an `auto` option that follows your system appearance:

| Theme | Description |
|-------|-------------|
| **dark** | Neutral dark base with Rho's Talon Gold accent. Default theme. |
...
Run the `/theme` slash command to open the theme picker. As you move through the list with the arrow keys, Rho previews each theme in real time:
...
Set the theme in `~/.rho/settings.json`:
...
Set `theme: "auto"` to have Rho follow your operating system's light/dark appearance and switch themes automatically:
...
Once running, Rho checks for appearance changes when you explicitly switch themes (no continuous polling to avoid resource drain).

---

## Color Support Detection

On startup, Rho detects your terminal's color capability:
...
When `COLORTERM=truecolor` is set, Rho uses truecolor mode. When `TERM` contains `256color`, it uses 256-color mode. When `NO_COLOR` is set, Rho renders in monochrome.
...
This shows detected color support, current settings, and tips for optimal Rho experience.
