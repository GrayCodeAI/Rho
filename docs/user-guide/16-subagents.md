...
When Rho needs to delegate work, it spawns a child session using the `Agent` tool. The child runs with:
...
Personas are behavioral overlays applied to subagents. Define them in `~/.rho/settings.json`:
