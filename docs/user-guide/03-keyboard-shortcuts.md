...
Reference for key bindings in the Rho TUI. Bindings are built-in and cannot currently be remapped.

---

## Input Modes

Rho has two input modes that control how you navigate the scrollback:
...
Or set `vim_mode = true` under `[ui]` in `~/.rho/settings.json`:
