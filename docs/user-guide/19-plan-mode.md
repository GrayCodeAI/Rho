...
Plan mode is a structured planning phase where Rho explores the codebase and designs an implementation before writing code. Use it when tasks have genuine ambiguity about the right approach.

---

## What Plan Mode Does

When plan mode is active, Rho:

1. Reads and searches the codebase to understand patterns
2. Designs an implementation approach
3. Writes the plan to a file under `.rho/specs/`
...
Plans are written to `.rho/specs/<slug>/plan.md`:
...
When planning completes, Rho opens a preview with action bar:
