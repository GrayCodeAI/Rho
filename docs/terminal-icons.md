...
Rho uses current Nerd Font Codicon glyphs for interactive terminal output.
...
RHO_ICONS=nerd ./bin/rho

# Portable text-only output
RHO_ICONS=ascii ./bin/rho
```

For the real icons, configure the terminal profile—not Rho's Go code—with a
patched font such as `JetBrainsMono Nerd Font` or `Symbols Nerd Font Mono`.
Font size and glyph scale are controlled by that profile. Rho applies bold
