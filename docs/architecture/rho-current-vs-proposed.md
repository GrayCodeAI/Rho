# Rho Current vs Proposed Architecture

## Purpose

This document explains the connected repository design. The canonical
machine-readable inventory is [`rho/ecosystem.yaml`](../../ecosystem.yaml);
...
| `graycode-platform` | GrayCode web, BFF, and Rho Cloud deployment |

## Current workspace

All directories listed in the manifest are independent Git repositories in the
plain `graycode-eco` workspace. The parent `go.work` (generated by
`make setup` / `scripts/generate-workspace.sh`) connects only the Go modules
marked `workspace: true` in the manifest:
...
└── graycode-platform    # web/BFF/Rho Cloud platform
```

There is no `rho/external` vendor tree in the current workspace. Local
...
rho ── optional, fail-open HTTP ──> graycode-platform/apps/worker
...
- Rho is the only orchestrator and product integration root.
- Engines are peers and do not import Rho internals or one another.
...
- SDKs consume Rho's daemon API contract; they do not import engines.
- Starling provides content and extension metadata through Rho's skill surface.
- GrayCode Platform is outside the Rho runtime module graph.
...
contract parity checks, and Rho-centered dependency direction all exist.
...
graycode-platform <-> Rho via authenticated HTTP/Service Binding only
```
...
SDK -> engine
skills -> engine
any Rho engine -> graycode-platform code
any Go module -> graycode-platform code
```

The sanctioned platform connection is runtime-only and HTTP-based: Rho may
send explicitly enabled usage or graph requests to the deployed Rho Cloud
Worker, and local execution must remain usable when that request fails.

## Recommendation

Keep the repository structure represented by `ecosystem.yaml`. Treat
