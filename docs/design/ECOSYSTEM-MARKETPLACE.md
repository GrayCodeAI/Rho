...
**Scope:** Multi-month effort spanning all 5 repos (rho, eyrie, harrier, shrike, swift) + a new gallery web property + a unified docs site.
...
The gap is therefore **consolidation and elevation**, not invention: (a) a *standardized, multi-asset* extension format (today's registry is skills-only); (b) a *browsable web gallery*; (c) a *trust/signing* story beyond static malware regexes; and (d) a *unified docs site* that today is five disconnected `README.md`/`ARCHITECTURE.md` trees (`rho/docs/`, `eyrie/docs/`, `harrier/`, `shrike/docs/`, `swift/docs/`).
...
│  cmd/skills_cmd.go (+ ext cmd)│      │  - /docs/{rho,eyrie,...}     │
...
```
CI (docs repo) →
  pull README.md / ARCHITECTURE.md / docs/** from each of rho, eyrie,
...
- **M1.1** Stand up the static site (decision in §6). Pages: `/extensions` (gallery), per-extension detail, `/docs/{rho,eyrie,harrier,shrike,swift}` (getting-started + API ref + architecture), `/comparison` (rendered from `TOP20_COMPARISON.md`).
