...
Rho is the product face; Eyrie is the provider engine. Rho owns the CLI/TUI,
...
Production Rho packages must not import Eyrie packages below
...
Eyrie engine and projects engine models into Rho UI and command contracts:
...
4. Advance Rho's `../eyrie` sibling checkout to that exact commit, then update
   Rho's module version when the Eyrie revision is published.
