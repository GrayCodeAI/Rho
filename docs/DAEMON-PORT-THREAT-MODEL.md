# Rho Daemon — Port 4590 Threat Model

The Rho daemon (`rho daemon`) binds an HTTP server on port **4590** (default)
...
**Mitigation:**
- Set `RHO_DAEMON_API_KEY` and configure the daemon with `--api-key` to
...
Any local process can detect that port 4590 is open and identify rho.
...
`~/.rho/sessions/rho.db`. A local attacker with filesystem read access can
...
- A global concurrency cap bounds in-flight generations (default **4**, tuned
  via `RHO_DAEMON_MAX_CONCURRENT`). When the cap is hit, new `/v1/chat`
...
- **Process privilege escalation**: rho does not run as root and does not use
  `setuid`/`setgid`.
...
export RHO_DAEMON_API_KEY="your-random-secret-here"
rho daemon

# Option 2: CLI flag
rho daemon --api-key "your-random-secret-here"
```
...
The SDK reads this from `RHO_DAEMON_API_KEY` automatically if the env var is
set.
...
```bash
rho daemon --port 9000
```

Or in `~/.rho/settings.json`:
```json
{
  "daemon": {
    "port": 9000
...
If rho is running on a machine shared by multiple OS users (e.g., a dev
...
# Linux — iptables: allow only the rho-owner UID
