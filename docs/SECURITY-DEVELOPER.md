...
This document describes how rho and eyrie handle API keys and agent isolation for an individual developer on macOS or Linux (no Vault, no proxy). Teams and enterprise deployment models come later.
...
- Rho talks to eyrie without putting keys in JSON or chat messages.
...
| Write | Read | Remove |
|-------|------|--------|
| `/config` paste flow → `eyrie/engine.Engine.SaveCredential` | `Engine.ResolveCredential` (secret store only) | `/config key remove` or `rho credentials remove` |
...
User pastes API key in /config
        |
        v
Rho /config -> Eyrie engine credential service (OS secret store)
        |
        v
...
- **Control plane**: Rho calls only `eyrie/engine`; no lower Eyrie package is a
  production import.
...
- **Chat**: Rho sends model intent, messages, and tool definitions; Eyrie
...
- **Read** tool: legacy Rho env files, Eyrie's configured `provider.json`,
  `~/.ssh/*`, etc.
...
- Rho: `internal/config/eyrie_engine.go`, `internal/tool/safety.go`,
