# Adopt durable-design ideas from deepseek-harness in rho
...
| `docs/capability-seams.md` | owner / impl / consumer roles | `docs/architecture/rho-capability-seams.md` (Phase 2) |
...
- `docs/architecture/rho-capability-seams.md`: owner / impl / consumer table.
...
These numbers compare Rho against the real deepseek-harness source, not the
plan's own scoped promises. The clone inspected was
`deepseek-ai/deepseek-harness` at `dsh-0.1.0-rc.7`.

Rho is a **deliberate subset**. It ports the session-log spine, the fail-closed
approval waterfall, and the interceptor/disposer seams; it does not port the
product-wide plugin catalogue. Measured head-to-head, Rho has roughly **20–30%**
of upstream by feature surface, and roughly **80–90%** of the plan's stated scope.

| Surface | DSH | Rho today | Gap |
| --- | --- | --- | --- |
...
- DSH `packages/llm/llm/src/assembler.ts` (response normalization — covered by Flux facade).
...
Each phase: `make ci` + `rho verify`; no direct commits to `main`.
