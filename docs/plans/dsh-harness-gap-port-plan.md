...
| # | DSH package(s) | Rho gap | Port status |
| --- | --- | --- | --- |
...
| 24 | `sdk/client`, `sdk/protocol`, `sdk/server`, `python/sdk` | rho exposes ACP server instead of DSH JSON-RPC SDK | deferred (keep ACP) |
...
- Resolved once per process (memoized): `$RHO_HOME`-style home (`~/.rho`),
...
- `attachment.Ref` (ID, media type, bytes, width, height, optional name),
  ...
  filesystem `Store` under the rho home data dir.
...
coordinator stays out of scope — rho has no Cordis bus; records reach the
...
- `DefaultConfig()` mirrors oteltrace env conventions
  (`RHO_ENABLE_TELEMETRY=1` + `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT`);
  wired in `cmd/rho/main.go` and `cmd/daemon.go`.
...
and `rho verify` — run **twice** (the second pass re-runs the full touched
...
- `go test -race` (identity, jobs, attachment): ok; `go test` (session, tool): ok
- `make lint`: 0 issues; `rho verify`: exit 0
...
- `gofmt -l` (cmd/rho/main.go, cmd/daemon.go, internal/observability/otellog): clean
