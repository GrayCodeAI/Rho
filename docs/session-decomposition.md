...
> Date: 2026-06-12
> Scope: `rho/internal/engine/session.go` (the 35-collaborator `Session` struct)

## Problem

`Session` at `rho/internal/engine/session.go:42-141` is a god object:
...
**Methods:**
- `BuildOptions(systemPrompt string, tools []client.FluxTool, activeModel string, maxTokens int) types.ChatOptions`
- `Chat(ctx, msgs, opts) (*types.FluxResponse, error)` — non-streaming, used by background goroutines
...
**File:** `rho/internal/engine/chat_service.go` (~150 LOC)
...
- `RunSleeptimeConsolidation(ctx, provider ChatService, messages []types.FluxMessage)` — background
...
**File:** `rho/internal/engine/memory_service.go` (~200 LOC)
...
**File:** `rho/internal/engine/tool_service.go` (~400 LOC — biggest, owns the 15-stage pipeline)
...
**File:** `rho/internal/engine/permission_service.go` (~150 LOC)
...
- `OnSessionEnd(ctx, success bool, duration time.Duration, messages []types.FluxMessage)`
- `SelectModel(taskType, currentModel, hint string) string` — wraps Cascade
- `OnToolFailure(ctx, intent string, history []types.FluxMessage, errOutput string) (*Reflection, error)`
- `RecordDecision(turn int, toolNames string, msgs []types.FluxMessage)`
...
- `InjectSteering(messages []types.FluxMessage) []types.FluxMessage`

**File:** `rho/internal/engine/lifecycle_service.go` (~250 LOC)
...
- `Messages() []types.FluxMessage`, `RawMessages()`, `MessageCount()`, `LoadMessages([]types.FluxMessage)`
- `RemoveLastExchange()`
...
**File:** `rho/internal/engine/persistence_service.go` (~300 LOC)
...
func (s *Session) agentLoop(ctx context.Context, ch chan<- StreamEvent) {
    ...
    for turn := 0; ; turn++ {
        ...
        opts := s.Chat.BuildOptions(s.system, s.Tools.Registry().FluxTools(), activeModel, maxTok)
...
6. **Phase 6**: extract `PersistenceService` (last because it owns the `messages []types.FluxMessage` field which is touched everywhere).
