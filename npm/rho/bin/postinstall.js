#!/usr/bin/env node
// Runs once after npm install/update. Reads the Rho binary from the matching
// per-platform optional dependency (@graycodeai/rho-<platform>-<arch>) and
// installs it to ~/.rho/bin/ using versioned filenames:
//
//   Unix:    rho-<version>  +  rho  (symlink)
//   Windows: rho-<version>.exe  +  rho.exe  (copy)
//
...
// the same versioned-at-~/.rho/bin/+symlink convention. Install either path;
// they converge on the same layout.
//
// Suppress entirely with RHO_SKIP_POSTINSTALL=1 (useful for CI / controlled
...
// Canonical install location mirrors install.sh (RHO_HOME default ~/.rho).
const RHO_HOME = process.env.RHO_HOME || path.join(os.homedir(), '.rho');
const CANONICAL_DIR = path.join(RHO_HOME, 'bin');
...
if (!SUPPORTED.has(key)) {
    console.error(`@graycodeai/rho: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@graycodeai/rho-${key}`;
    try {
...
if (!version) {
    console.error('@graycodeai/rho: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (fs.existsSync(brPath)) {
        ...
        console.error(`@graycodeai/rho: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@graycodeai/rho: failed to update ${canonicalPath}: ${e2.message}`);
                console.error('Close all running rho processes and try again.');
                return false;
            }
        }
    } else {
...
if (process.env.RHO_SKIP_POSTINSTALL === '1') {
    console.log('@graycodeai/rho: RHO_SKIP_POSTINSTALL=1 set; skipping binary install.');
    process.exit(0);
}

const platformDir = resolvePlatformPackageDir();
if (!platformDir) {
    console.error(`@graycodeai/rho: platform package @graycodeai/rho-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @graycodeai/rho');
    process.exit(0);
}

installBinary('rho', platformDir, `rho${EXE}`);
cleanupOldVersions('rho');

console.log('');
if (process.env.RHO_SKIP_HINT !== '1') {
    console.log(`Add ${CANONICAL_DIR} to your PATH if it is not already, e.g.`);
    console.log(`  export PATH="$PATH:${CANONICAL_DIR}"`);
    console.log('');
    console.log('Restart any running rho sessions to pick up the new binary — the old');
