...
# sync-submodules` + a commit in rho to bump the external/tok pin. This
...
if [[ $exit_code -ne 0 ]]; then
  echo
  echo "drift detected — run 'make sync-submodules' in rho after confirming the sibling repos are what you expect, then commit the updated external/ pins"
fi
