...
This directory is that source of truth. It lives here, in `rho`, because
`rho` is the one repo every consumer already depends on or references —
there is no separate monorepo at the workspace root to hold it.

**This directory is not built or run by rho itself.** It is a template
...
- `Makefile.binary.tmpl` — Go binary repos (currently only `rho`)
...
- `scripts/sync-external.sh` — read-only drift report for `rho`'s `external/` submodule pins (rho-specific, not templated elsewhere)
...
per repo. `rho`'s own `Makefile`/`lefthook.yml`/CI predate this directory
and intentionally diverge in a few binary-specific ways (see
`Makefile.binary.tmpl`, which documents the real deltas against `rho`'s
Makefile).
