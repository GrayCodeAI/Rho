...
func (m chatModel) loadConfigGatewayRows() []configGatewayRow {
	...
	statuses := rhoconfig.GatewayStatuses(ctx, active, activeModel)
	...
	for _, status := range statuses {
		...
		if hasKey {
			credentialEnv, keyConflict = rhoconfig.CredentialEnvironmentConflict(ctx, status.ID)
		}
		display := status.DisplayName
		if status.RegionRequired || status.RegionLabel != "" || rhoconfig.HasRegionOptions(status.ID) {
		...
		rows = append(rows, configGatewayRow{
			...
			ModelCount:     count,
			Active:         status.Active || rhoconfig.ActiveProviderID(status.ID) == rhoconfig.ActiveProviderID(active),
...
func (m chatModel) configGatewaysView() string {
	...
	if m.configKeysPendingRemove != "" {
		name := rhoconfig.GatewayDisplayName(m.configKeysPendingRemove)
		b.WriteString("\n" + mutedStyle.Render(indent+configGatewayRemovePrompt(m.configKeysRemoveStep, name)))
	} else if !rhoconfig.HasConfiguredDeploymentCached(ctx) {
		hint := "Select a gateway · enter · paste API key · then Models tab"
		if targetIdx >= 0 && targetIdx < len(rows) && (rows[targetIdx].RegionRequired || rows[targetIdx].RegionLabel != "" || rhoconfig.HasRegionOptions(rows[targetIdx].ID)) {
...
func (m chatModel) handleConfigGatewaysSelect() (chatModel, tea.Cmd) {
	...
	if (row.RegionRequired || rhoconfig.HasRegionOptions(row.ID)) && (!row.HasKey || row.RegionRequired) {
	...
	_ = rhoconfig.SetGlobalSetting("provider", gw)
	if active := rhoconfig.ActiveProvider(context.Background()); active != "" {
...
func refreshGatewayAsync(providerID string) tea.Cmd {
	return func() tea.Msg {
		summary, err := rhoconfig.RefreshGatewayCatalog(context.Background(), providerID)
...
func (m chatModel) handleConfigGatewayRefreshMsg(msg configGatewayRefreshMsg) chatModel {
	...
	if msg.err != nil {
		m.configNotice = sanitizeConfigNotice(rhoconfig.FormatConfigProviderError(msg.providerID, msg.err))
		return m
	}
