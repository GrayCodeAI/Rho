...
import (
	...
	"github.com/GrayCodeAI/rho/internal/recipe"
)
...
func (r *recipeSubcommand) Handle(m *chatModel, args []string, text string) (tea.Model, tea.Cmd) {
	...
	if arg == "" || arg == "list" {
		...
		recipes := rn.List()
		if len(recipes) == 0 {
			m.messages = append(m.messages, displayMsg{role: "system", content: "No recipes found in Rho user state or .agents/recipes/"})
		} else {
			var list string
