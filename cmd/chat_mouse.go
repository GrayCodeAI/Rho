...
func mouseEnabledFromEnv() bool {
	v := strings.TrimSpace(os.Getenv("RHO_MOUSE"))
...
// "mouse": false). Priority: RHO_MOUSE env → runtime override → settings → default on.
...
func (m *chatModel) handleMouseCommand(parts []string) {
	...
	if envOverridesMouse() {
		m.messages = append(m.messages, displayMsg{
			role: "system",
			content: "Mouse is controlled by RHO_MOUSE env in this session. " +
	...
	case "on", "true", "1", "enable":
		...
		_ = rhoconfig.SetGlobalSetting("tui_mouse", "true")
	...
	case "off", "false", "0", "disable":
		...
		_ = rhoconfig.SetGlobalSetting("tui_mouse", "false")
		m.messages = append(m.messages, displayMsg{
			role:    "system",
	...
	case "toggle":
		...
		_ = rhoconfig.SetGlobalSetting("tui_mouse", val)
