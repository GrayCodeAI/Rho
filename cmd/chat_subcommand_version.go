...
// the running rho version. Follows the SubcommandRegistry pattern
...
func (v *versionSubcommand) Description() string { return "print the running rho version" }
...
func (v *versionSubcommand) Handle(m *chatModel, args []string, text string) (tea.Model, tea.Cmd) {
	m.messages = append(m.messages, displayMsg{role: "system", content: fmt.Sprintf("rho v%s", DisplayVersion())})
	return m, nil
}
