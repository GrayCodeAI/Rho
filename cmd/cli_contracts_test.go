...
func TestPrepareSession_ResumeUsesRecoveryPath(t *testing.T) {
	...
	walPath := filepath.Join(os.Getenv("RHO_STATE_DIR"), "sessions", saved.ID+".wal")
...
func TestReplBuiltinResponse_ToolsAndSession(t *testing.T) {
	...
	toolsOut, handled, err := replBuiltinResponse("/tools", sess, rhoconfig.Settings{}, "session-1")
	if err != nil {
	...
	sessionOut, handled, err := replBuiltinResponse("/session", sess, rhoconfig.Settings{}, "session-1")
	if err != nil {
...
func TestReplBuiltinResponse_Models(t *testing.T) {
	...
	out, handled, err := replBuiltinResponse("/models", sess, rhoconfig.Settings{}, "session-1")
	if err != nil {
