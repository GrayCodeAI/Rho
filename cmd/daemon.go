...
var daemonCmd = &cobra.Command{
	Use:   "daemon",
	Short: "Manage the rho background server",
	Long:  "Run rho as a background HTTP server for programmatic/CI access.",
}
...
func init() {
	...
	daemonStartCmd.Flags().StringVar(&daemonAPIKey, "api-key", "", "API key for protected daemon endpoints (defaults to RHO_DAEMON_API_KEY or a generated key)")
...
func runDaemonStart(_ *cobra.Command, _ []string) error {
	...
	// ~/.rho/state/daemon.log with slog structured output.
	...
	newSession := newConfiguredRhoSessionFactory(settings, daemonLogger)

	// Log startup banner.
	daemonLogger.Info("rho daemon starting", map[string]interface{}{
		"host":              daemonHost,
		"port":              daemonPort,
	...
	apiKey := daemonAPIKey
	if apiKey == "" {
		apiKey = os.Getenv("RHO_DAEMON_API_KEY")
	}
	if apiKey == "" {
		var err error
	...
	// Wire Flux's authoritative local preflight into GET /v1/ready. A session
	// factory only proves Rho can attempt construction; readiness additionally
	// requires Flux's provider state, catalog, credentials, and model selection.
	...
	fmt.Printf("rho daemon running on http://%s\n", addr)
	...
	if len(apiKey) > 8 {
		...
		fmt.Println("API key: (set via --api-key or RHO_DAEMON_API_KEY)")
	}
	...
	if telemetryProviders != nil && telemetryProviders.IsEnabled() {
		...
		fmt.Println("Telemetry: disabled (set RHO_ENABLE_TELEMETRY=1 to enable)")
	}
...
// ~/.rho/state/daemon.log and returns it. The directory is created if needed.
...
// performs Flux's local preflight (provider state, catalog, credentials, and
...
//   - Preflight reports incomplete   -> not ready with the failed Flux check.
//
// This never performs a paid/live model call — Flux preflight only inspects
...
func daemonReadyProbe(factory daemon.SessionFactory) func() (bool, string) {
	return daemonReadyProbeWithPreflight(factory, rhoconfig.EnginePreflightReport)
}

func daemonReadyProbeWithPreflight(factory daemon.SessionFactory, preflight func(context.Context) rhoconfig.EnginePreflight) func() (bool, string) {
	return func() (bool, string) {
		...
		for _, check := range report.Checks {
			if string(check.Status) == "fail" {
				...
				return false, fmt.Sprintf("Flux %s: %s", check.Name, detail)
			}
		}
		if ctx.Err() != nil {
			return false, "Flux preflight timed out"
		}
		return false, "Flux preflight is not ready"
	}
}
...
// daemonAutonomyFromFlag resolves the --autonomy flag / RHO_DAEMON_AUTONOMY
