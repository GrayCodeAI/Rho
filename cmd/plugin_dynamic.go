...
var pluginStatusCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if len(statuses) == 0 {
			cmd.Println(auditTint("No plugins discovered. Run 'rho plugin install' to add plugins.", textMuted))
			return nil
		}
...
var pluginCreateCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		manifest := &plugin.ManifestV2{
			Name:        name,
			Version:     "0.1.0",
			Description: fmt.Sprintf("A rho plugin: %s", name),
			Author:      "",
			Mode:        "subprocess",
...
`, name, name)
		...
		cmd.Printf("%s\n", auditTint("  rho plugin install ./"+name, textMuted))
		cmd.Printf("%s\n", auditTint("  rho plugin activate "+name, textMuted))
		return nil
	},
}
...
var pluginMarketplaceListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, _ []string) error {
		...
		if err != nil {
			prog.Abort()
			return fmt.Errorf("fetch marketplace: %w (indexes may be unpublished; add a source with rho plugin marketplace add)", err)
		}
		prog.CompleteStep(0)
		prog.Done()
		if len(entries) == 0 {
			cmd.Println(auditTint("No marketplace plugins found.", textMuted))
			cmd.Println(auditTint("Add a source: rho plugin marketplace add <name> <index-url>", textMuted))
			return nil
		}
