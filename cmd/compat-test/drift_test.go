...
require github.com/GrayCodeAI/other v1.9.0 // indirect
`)
...
// TestCheckDrift reports drift when a consumer pins an older version than rho.
func TestCheckDrift_DetectsDrift(t *testing.T) {
	ws := t.TempDir()
	writeMod(t, filepath.Join(ws, "rho", "go.mod"), `module github.com/GrayCodeAI/rho
...
`)
	...
	r, w, _ := os.Pipe()
	os.Stdout = w
	err := checkDrift(filepath.Join(ws, "rho"))
	_ = w.Close()
	os.Stdout = old
	_, _ = buf.ReadFrom(r)
	...
	if !strings.Contains(buf.String(), "consumer") {
		t.Errorf("expected drift report to mention consumer, got:\n%s", buf.String())
	}
}
