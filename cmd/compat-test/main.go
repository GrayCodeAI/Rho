...
//	go run ./cmd/compat-test -check-external # advisory: compare rho's own
...
// It must not pick rho/platform-capabilities.json, which is a different document.
func findMatrixFile() string {
	...
	candidates := []string{
		...
		"rho/testdata/compatibility-matrix.json",
	}
	for i := 0; i < 6; i++ {
