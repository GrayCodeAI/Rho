...
check_layer() {
  ...
  local forbidden=$2
  local matches
  matches=$(git grep -n -E "github.com/GrayCodeAI/rho/(${forbidden})([\"/])" -- "${directory}/**/*.go" \
...
if ((failed)); then
  echo "Rho internal layer boundary check failed" >&2
  exit 1
fi
echo "Rho internal layer boundary check passed"
